...
class PackageListViewModel {
    ...
    let packages = Variable<PackageListViewPackagesState>(.loading)
    let dateText = Variable<String>("Your Updates")
    let stats = Variable<Statistics>(Statistics(awaiting:0,delivered:0,traveling:0))
...
extension PackageListViewModel {
    ...
    func setDateText() {
        ...
        if let hour = hour {
            if hour < 12 {
                dateText.value = "Your Morning Updates"
            }
            if hour >= 12 && hour < 20 {
                dateText.value = "Your Afternoon Updates"
            }
            if hour >= 20 && hour <= 24 {
                dateText.value = "Your Evening Updates"
            }
        }
    }
    ...
    func firebaseListen() {
        ...
        initiateFirebaseListen()?.flatMap({ [weak self] id -> Observable<[String:AnyObject]> in
            ...
            self?.generateStatistics(packages)
